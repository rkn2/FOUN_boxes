...
\begin{abstract}
This study presents a data-driven approach to assess structural vulnerability and develop intervention strategies for adobe structures, using Fort Union National Monument (FOUN) as a case study. The research integrates multiple analytical methods, including correlation analysis, Factor Analysis, and Random Forest machine learning, to identify and prioritize damage conditions affecting adobe structure behavior. 
Data collection involved on-site surveys (n=67 wall sections) with archival research and LiDAR scanning. Factor Analysis revealed three distinct latent structures of degradation—Sill Deterioration, Surface/Lintel Interaction, and Structural Instability—together explaining 65\% of variance. 
Random Forest analysis revealed that visible damage indicators (Coat 2 Cracking, Structural Cracking, Out of Plane movement) were the strongest predictors in the full model (R²=0.52), reflecting analytical circularity where aggregate scores are predicted by their components. A supplemental model using only geometric and contextual features (excluding all damage indicators) achieved R²=0.29, identifying LiDAR-derived surface irregularity, wall height, and foundation height as key intrinsic vulnerability factors. This 56\% retention of predictive performance enables preventive risk stratification independent of current damage state.
Intervention matrices combining statistically-derived priorities with Secretary of Interior preservation standards provide a structured framework for prioritizing preservation actions. 
Limitations include small sample size (n=67), single-site scope constraining generalizability, and cross-sectional data precluding causal inference about treatment effectiveness. The methodology offers a replicable framework for adobe assessment, though validation at additional sites and longitudinal monitoring are needed.
...
\hl{Becca: please replace this example with actual details - surveys conducted by [team size? single observer or multiple?], date[s] of surveys, and inter-rater reliability results if tested}
...
Building upon the insights gained from both correlation analysis and Factor Analysis, we further refined our understanding by employing Random Forest regression. 
...
The Random Forest model was implemented using scikit-learn in Python with the following hyperparameters: 100 trees (n\_estimators=100), maximum depth unrestricted to capture full complexity, minimum samples per split of 2, and random state of 42 for reproducibility. 
Given the limited sample size (n=67 wall sections, 28 features), we employed 5-fold cross-validation to assess model performance and reduce overfitting risk. 
The data were not split into separate train/test sets due to sample size constraints; instead, cross-validation provided out-of-sample performance estimates.

Model performance metrics indicated moderate predictive accuracy with cross-validation R² = 0.52 (±0.12), RMSE = 8.21, and MAE = 6.76. 
The out-of-bag R² score was 0.60, slightly higher than cross-validation performance, suggesting reasonable model stability. 
The training R² (0.95) substantially exceeded cross-validation R² (0.52), indicating some degree of overfitting as expected given the feature-to-sample ratio of approximately 1:2.4. 
To ensure the reliability of feature importance rankings despite the small dataset, we computed permutation importance scores with 95\% confidence intervals via bootstrapping (1000 iterations). 
We note that with a feature-to-sample ratio approaching 1:2, there is elevated risk of overfitting, and importance scores should be interpreted as associations rather than definitive causal rankings.

Random Forest offers several advantages in this context: it tolerates multicollinearity, handles both continuous and categorical predictor variables, and provides measures of variable importance allowing us to rank the factors most influential in predicting degradation. 
Mean decrease in impurity (MDI) can be biased toward high-cardinality features or continuous variables with many unique values. Permutation importance provides an alternative unbiased metric.
...
All p-values were adjusted using the Bonferroni correction to account for multiple comparisons (corrected α = 0.001 for 55 pairwise comparisons). 
...
The correlation between Bracing and Total Scr (r = 0.14, p = 0.24) warrants particular attention. 
...
Prior to factor extraction, we verified that the data were suitable for factor analysis. 
The Kaiser-Meyer-Olkin (KMO) measure of sampling adequacy was 0.558, falling slightly below the conventional 0.6 threshold (Kaiser's "mediocre" classification).
We proceeded with Factor Analysis for several reasons: (1) Bartlett's test strongly rejects variable independence (χ² = 376.04, df = 28, p < 0.001), (2) individual variable KMO values for key damage indicators exceed 0.6 (Lintel Deterioration: 0.66, Coat 2 Cracking: 0.76, Structural Cracking: 0.61; 4 of 8 variables exceed 0.6 threshold), (3) the extracted factors align with theoretically meaningful damage mechanisms, and (4) the small sample size (n=67) may artificially depress KMO values in exploratory analyses.
...
\hl{Becca: Alternative approaches to consider - hierarchical clustering of correlation matrix or network analysis showing damage co-occurrence patterns could complement FA findings without requiring KMO assumptions}

A three-factor model with varimax rotation was selected based on eigenvalue-greater-than-one rule (Kaiser criterion), scree plot inflection point, and interpretability of factor structure. 
The three extracted factors explained 65.1\% of the total variance, which is considered adequate for exploratory factor analysis (typical threshold: >60\%), with individual factor contributions of 26.1\% (Factor 1), 24.9\% (Factor 2), and 14.0\% (Factor 3). 
...
Out of Plane & 1.00 \\
\bottomrule
\multicolumn{2}{l}{\footnotesize $^{*}$Heywood case (communality >1.0);} \\
\multicolumn{2}{l}{\footnotesize indicates strong factor loading but} \\
\multicolumn{2}{l}{\footnotesize highlights exploratory nature of solution} \\
\multicolumn{2}{l}{\footnotesize \cite{DillonGoldstein1984}.} \\
\end{tabular}
\end{table}

Communalities (proportion of variance explained by the 3-factor solution) are reported in Table \ref{tab:communalities}, with most values exceeding 0.4, indicating adequate representation by the factor solution. 
Coat 1 Cracking exhibits a Heywood case (communality slightly >1.0), a known phenomenon in exploratory factor analysis with small samples that does not invalidate the overall structure but reinforces the exploratory interpretation of results.
The factor loadings are presented in Table \ref{tab:fa_loadings}.

The first factor is heavily loaded on Sill 1 (0.91) and Sill 2 (0.98). 
We acknowledge the reviewer's concern that this factor may primarily reflect measurement redundancy (sill condition assessed on opposite orientations of the same wall) rather than a distinct latent construct. 
...
The term "Architectural Detail Vulnerability" reflects that sills function as water-shedding elements at the base of openings; their deterioration, while often localized, can facilitate water ingress into the wall core.

The second factor, Surface and Lintel Interaction, exhibits high loadings for Coat 1 Cracking (0.72) and Lintel Deterioration (0.61). 
...
We hypothesize this reflects orientation-specific exposure: \\hl{Becca: please investigate - does Orientation 2 face prevailing winds or more severe weather? Check site plans and meteorological data if available}. 
...
The top three predictors of degradation were Coat 2 Cracking (importance = 0.207, permutation importance = 0.251 ± 0.042), Structural Cracking (0.176, perm = 0.226 ± 0.046), and Out of Plane movement (0.115, perm = 0.136 ± 0.020) as shown in Figure \ref{fig:random_forest_new}. 
Among geometric features, Height showed moderate importance (0.055, perm = 0.032 ± 0.006), ranking 5th overall, while Foundation Height had lower importance (0.024, perm = 0.013 ± 0.003).
These results partially diverge from correlation analysis, where geometric factors showed strong associations with Total Score but have moderate predictive importance when accounting for other damage indicators. 

The dominance of visible damage indicators (Coat 2 Cracking, Structural Cracking, Out of Plane) as top predictors reflects the Random Forest model's use of already-manifest damage to predict overall degradation scores. 
This is mechanistically sensible: Total Score aggregates multiple damage types, so features representing severe damage (cracking, leaning) naturally correlate strongly with that aggregate.
However, for intervention planning, this creates a circular dependency—we cannot use "presence of structural cracking" to predict "total degradation including structural cracking."

While this circularity limits analytical utility, it offers practical value: field managers can rapidly prioritize walls for detailed assessment based on readily observable cracking and leaning, which strongly predict overall condition (R² = 0.52 with damage indicators included). 
This finding suggests that rapid visual triage focusing on visible structural damage captures the majority of information that comprehensive surveys would reveal.

The moderate importance of geometric features (Height, Foundation Height) is more actionable for preventive prioritization. 
While they rank 5th and lower, their importance persists even after accounting for existing damage, suggesting they represent intrinsic vulnerability rather than merely describing current state.
This interpretation aligns with structural mechanics:

\subsubsection{Supplemental Analysis: Geometric Factors Without Damage Confounding}

To isolate the predictive power of geometric and contextual features without damage indicator confounding, we conducted a supplemental Random Forest analysis predicting Total Score using only non-damage variables: Height, Foundation Height, Treatment, Bracing, Bracing Score, Animal Activity, Fireplace, Point Cloud Mean, and Point Cloud Deviation (n=9 features).

This geometric-only model achieved CV R² = 0.29 (±0.15), RMSE = 10.22, MAE = 8.54, and OOB R² = 0.34, representing reduced but non-trivial predictive performance compared to the full model (CV R² = 0.52).
Feature importance rankings shifted dramatically (Figure \ref{fig:random_forest_geometric}):

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{Images/feature_importance_geometric_only.png}
    \caption{Supplemental Random Forest Feature Importance: Geometric/Contextual Factors Only (excluding all damage indicators).}
    \label{fig:random_forest_geometric}
\end{figure}

Point Cloud Mean emerged as the dominant predictor (MDI = 0.264, perm = 0.407 ± 0.068), followed by Height (MDI = 0.204, perm = 0.322 ± 0.060) and Foundation Height (MDI = 0.194, perm = 0.277 ± 0.042).
...
The strong performance of LiDAR-derived metrics (Point Cloud Mean and Deviation) suggests that geometric irregularities captured by high-resolution scanning contain information about structural condition not fully represented by simple height measurements.
...
This supplemental analysis demonstrates that geometric factors alone predict approximately 56\% of the variance explained by the full model (R² ratio: 0.29/0.52), confirming their value for preventive risk assessment in the absence of detailed damage surveys.
For managers planning prioritization without comprehensive condition assessments, walls exhibiting high Point Cloud Mean values (indicating surface irregularity), tall height, and exposed foundations warrant elevated monitoring attention.

\subsubsection{Structural Engineering Interpretation of Geometric Risk Factors}


\textbf{Wall Height Effects:}
Taller walls experience increased vulnerability through multiple mechanisms. Overturning moment scales with height squared (M $\propto$ h²); a wall twice as tall experiences four times the base moment under equal wind pressure. \hl{Becca: what are typical wall heights? Please report mean, range, and h/t (height-to-thickness) ratios.}
Adobe walls, as unreinforced masonry susceptible to buckling, become increasingly unstable as h/t ratios approach thresholds near 10-12 for unreinforced masonry. \hl{Becca: calculate h/t for tallest walls - do any exceed 10-12, typical thresholds for unreinforced masonry stability?}
Upper wall portions experience concentrated wind-driven rain without protection from adjacent structures.
Self-weight induced compressive stress at the base increases with wall height, though for these moderate heights (<4m typically), this is unlikely to approach adobe compressive capacity (\hl{Becca: do you have compressive strength test data for FOUN adobe?}).

\textbf{Foundation Height Mechanisms:}
Exposed foundation height may be associated with degradation through multiple physical pathways.
Exposed foundations experience direct impact from precipitation rebounding off ground surfaces, concentrating moisture at the wall-foundation interface. \hl{Becca: do you have any measured splash-back heights or erosion rate data?}
...
Future monitoring should measure soil moisture content adjacent to foundations, foundation bearing pressures, and settlement rates via repeated LiDAR scanning.

\textbf{Treatment Variable Interpretation:}
The low importance scores for specific historical treatments (Treatment: 0.003, Bracing: 0.001, Bracing Score: 0.004) indicate minimal predictive power. 
...
Historical treatments may have initially succeeded but degraded over the intervening decades. \hl{Becca: do you have treatment dates? If so, analyze degradation vs. time-since-treatment.}
With only \hl{Becca: how many walls received each treatment type?} treated walls per category, detecting treatment effects requires larger samples.
...
Based on the Factor Analysis and Random Forest results, we identified four primary conditions warranting intervention focus:

\textbf{(1) Structural Instability (Factor 3, RF rank 2-3):} This encompasses Structural Cracking (RF importance 0.176) and Out of Plane movement (0.115), grouping into Factor 3 alongside Coat 2 Cracking. 
...
\textbf{(2) Surface Coating Degradation (Factor 3, RF rank 1):} Coat 2 Cracking emerged as the single strongest predictor (0.207), loading heavily on Factor 3. 
...
\textbf{(3) Sill Deterioration (Factor 1):} This represents a distinct, localized vulnerability (Sill 1 importance 0.036, Sill 2 0.021). 
...
\textbf{(4) Surface and Lintel Interaction (Factor 2):} Coat 1 Cracking (0.029) and Lintel Deterioration (0.035) group as Factor 2, suggesting coupled failure where coating damage and lintel decay reinforce one another, particularly around openings.
...
\textbf{NSC Priority Scores (0-10):} Each NSC receives a priority ranking derived from the statistical analyses.
For NSCs corresponding to Factor Analysis factors, priority is calculated using a combined approach that considers both the explained variance of the factor and the strength of the leading variable within that factor.
For NSCs identified via Random Forest, priority equals 10 multiplied by (feature importance divided by maximum importance). 
The two scores are averaged if an NSC appears in both analyses.

\textit{Worked Example - Structural Instability (Factor 3):}
\begin{itemize}
    \vspace{-0.5em}
    \item Factor 3 explains 14.0\% of variance (Factor 1 explains maximum: 26.1\%)
    \item Highest loading in Factor 3: Coat 2 Cracking (0.59)
    \item Maximum loading across all factors: Coat 1 Cracking on Factor 1 (1.00)
    \item Variance contribution normalized: 14.0/26.1 = 0.536
    \item Loading strength normalized: 0.59/1.00 = 0.590
    \item Combined metric: (0.536 + 0.590)/2 = 0.563
    \item Priority score: 10 $\times$ 0.563 = 5.6, rounded to 6
    \item Cell score for mitigation: 6 (priority) $\times$ 6 (mitigation score) = 36
\end{itemize}
\vspace{-0.5em}

This scoring approach ensures that factors with strong loadings and substantial variance contributions receive higher prioritization, even if not the dominant factor.

\textit{Note on Priority Score Adjustment:} While the worked example above yields a calculated priority of 6, the final priorities assigned in Tables \ref{tab:intervention_matrix_adobe} and \ref{tab:intervention_matrix_foundation} incorporate adjustment based on preservation practice considerations and NPS management priorities. Structural Instability receives priority 9 (vs. calculated 6) due to immediate structural safety concerns and cascading effects on other damage types. Sill Deterioration receives priority 8 (vs. calculated value based on 26.1\% variance explained) recognizing its role as a primary water ingress pathway. These adjustments ensure intervention priorities align with both statistical significance and operational preservation practice.

\textbf{Preservation Standards Scores (0-6):} Each intervention approach is assigned a score reflecting compatibility with the Secretary of the Interior's Standards for the Treatment of Historic Properties \cite[]{NPSStandards}.
...
\textbf{Cell Score Calculation:} Each matrix cell score = (NSC priority) × (intervention approach score). 
...
\textit{Example}: Structural Instability (priority 9) × Mitigation-Shelter Coat (score 6) = 54, suggesting this should be a top funding priority.

We acknowledge this multiplicative weighting system represents one possible scoring framework. 
Sensitivity analysis \\hl{Becca: did you test alternative weighting schemes, e.g., additive or different preservation score distributions? If so, report whether rankings changed.} would strengthen confidence in these prioritizations. 
Future extensions could incorporate multi-criteria decision analysis (MCDA) methods such as Analytic Hierarchy Process (AHP) to formalize stakeholder preferences.
...
With n=67 wall sections and approximately 30 features, the feature-to-sample ratio approaches 1:2, raising concerns about overfitting in the Random Forest model. 
While cross-validation mitigates this risk, the limited sample size reduces statistical power to detect subtle effects, particularly for treatment variables where only \\hl{Becca: how many walls per treatment type?} sections received each intervention type. 
...
\textbf{Single-Site Generalizability.}
This case study focuses exclusively on Fort Union National Monument. 
While the methodological framework is transferable, the specific findings (e.g., foundation height as primary predictor) may reflect FOUN-specific conditions such as local climate \\hl{Becca: annual precipitation? freeze-thaw cycles?}, soil properties, construction techniques, and maintenance history. 
...
\textbf{Cross-Sectional Data Limitations.}
...
Critical questions remain unanswered: (1) Are high-degradation walls deteriorating faster, or have they merely persisted longer in poor condition? (2) Did bracing successfully arrest movement, or were already-stable walls selectively braced? (3) Have treatments extended service life, even if degradation scores remain elevated?
Longitudinal monitoring with repeated LiDAR scans and condition surveys (recommended interval: 5 years \\hl{Becca: is NPS planning any repeat surveys? If discussions exist, mention here}) would enable survival analysis and causal inference methods.

\textbf{Lack of Experimental Validation.}
...
A controlled intervention study, treating matched wall sections with different approaches and monitoring outcomes, would provide this validation but requires multi-year commitment \\hl{Becca: is NPS interested in pilot validation study?}.

\textbf{Observer Bias and Inter-Rater Reliability.}
The RAS damage scores (0-5 scale) involve subjective visual assessment. \\hl{Becca: how many raters? single observer or team? any inter-rater reliability testing?}
Without documented inter-rater reliability (Cohen's kappa or intraclass correlation), we cannot quantify measurement error. 
Systematic observer bias could inflate or deflate damage scores for particular features, affecting factor loadings and feature importance rankings. 
The illustrated glossary (Figure \ref{fig:RASsample}) provides standardization, but formal calibration exercises are recommended.

\textbf{Environmental Data Gap.}
The analysis lacks integration of climate and microclimate variables (precipitation, wind speed/direction, solar radiation, freeze-thaw cycles) despite these being hypothesized drivers of degradation. 
Orientation 2's association with structural instability (Factor 3) may reflect directional weather exposure, but without meteorological data, this remains speculative. 
Future work should incorporate: (1) historical weather station data, (2) microclimate sensors at wall locations, and (3) spatial analysis testing whether damage patterns correlate with aspect, elevation, or exposure indices.

\textbf{LiDAR Data Underutilization.}
Point cloud data yielded only two features (deviation, mean), when far richer geometric information could be extracted: wall lean angles, surface roughness, erosion depth mapping, foundation settlement patterns, and volume loss quantification. 
Advanced point cloud processing (e.g., change detection with historical photography, Structure-from-Motion photogrammetry) could substantially enhance the dataset \\hl{Becca: are raw point cloud data available for reanalysis?}.

\textbf{Material Property Unknowns.}
We lack laboratory test data for FOUN adobe: compressive strength, modulus of elasticity, moisture diffusivity, freeze-thaw resistance. 
These properties govern structural capacity and degradation susceptibility, yet their absence forces reliance on generic adobe properties from literature. 
Non-destructive testing (Schmidt hammer, ultrasonic pulse velocity) or minimally-invasive coring (with NPS approval) could characterize material variability across the site.

\textbf{Structural Mechanics Simplification.}
...
Integration of geotechnical investigation (soil bearing capacity, settlement potential) with structural analysis would provide rigorous basis for prioritizing structurally critical sections.

\textbf{Cost-Benefit Analysis Absent.}
The intervention matrices prioritize based on degradation severity and preservation standards but do not incorporate economic constraints. 
...
NPS facility managers need this economic dimension for practical decision-making \\hl{Becca: could rough cost estimates be obtained from NPS historical project records?}.

Addressing these limitations represents a roadmap for advancing data-driven heritage preservation methodology.

\section{Conclusions}

This study applied correlation analysis, Factor Analysis, and Random Forest regression to 67 wall sections at Fort Union National Monument to identify patterns of degradation and inform intervention prioritization. The multivariate analyses converged on three principal damage modes: Sill Deterioration (Factor 1), Surface and Lintel Interaction (Factor 2), and Structural Instability (Factor 3), together explaining 65\% of variance in the degradation data. These clusters suggest distinct failure mechanisms requiring different treatment strategies. Localized sill repairs may address water-shedding element deterioration, while structural interventions must tackle underlying settlement or lateral restraint loss.

Random Forest analysis revealed that visible damage indicators dominated feature importance in the full model (Coat 2 Cracking: 0.21, Structural Cracking: 0.18, Out of Plane: 0.11), reflecting analytical circularity where aggregate scores are predicted by their component damages. To isolate intrinsic vulnerability factors, a supplemental analysis using only geometric and contextual features (excluding all damage indicators) achieved CV R²=0.29, with LiDAR-derived Point Cloud Mean (surface irregularity), Height, and Foundation Height emerging as top predictors. This 56\% retention of predictive performance (relative to the full model's R²=0.52) confirms geometric factors' value for preventive risk stratification when comprehensive damage data are unavailable. The two-model approach resolves the analytical circularity while preserving the practical utility of damage-based rapid triage.

Treatment variables (bracing, historical interventions) showed negligible importance (<0.005), consistent with selection bias. Treatments were applied reactively to already-degraded walls; cross-sectional data cannot disentangle this confounding from treatment efficacy. Longitudinal monitoring with treatment timing data could resolve this through time-series analysis or propensity score matching.
...
Transferability to other adobe sites depends on several factors. The rapid assessment protocol and statistical workflow are generalizable; site-specific calibration would require adjustment for local climate, construction methods, and treatment history. Extensions incorporating temporal monitoring, climate scenario modeling, and cost data would strengthen decision-support utility. The limitations outlined in Section 6—particularly sample size, single-site scope, KMO marginality, and material property unknowns—constrain extrapolation beyond FOUN without additional validation studies.
